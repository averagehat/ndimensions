export const og_list = [
  ` The new form of the problem can be described in terms of a game which we call the 'Imitation Game.' 
  
  It is played with three people, a man (A), a woman (B), and an interrogator (C) who may be of either sex. The interrogator stays in a room apart front the other two. The object of the game for the interrogator is to determine which of the other two is the man and which is the woman. He knows them by labels X and Y, and at the end of the game he says either "X is A and Y is B" or "X is B and Y is A." The interrogator is allowed to put questions to A and B thus:

  C: Will X please tell me the length of his or her hair?

  Now suppose X is actually A, then A must answer. It is A's object in the game to try and cause C to make the wrong identification. His answer might therefore be: "My hair is shingled, and the longest strands are about nine inches long."
  
  In order that tones of voice may not help the interrogator the answers should be written, or better still, typewritten. The ideal arrangement is to have a teleprinter communicating between the two rooms. Alternatively the question and answers can be repeated by an intermediary. 
  
  The object of the game for the third player (B) is to help the interrogator.  The best strategy for her is probably to give truthful answers. She can add such things as "I am the woman, don't listen to him!" to her answers, but it will avail nothing as the man can make similar remarks. 
`,
  `In the sawdust-shrouded workshop, father, a titan of timber, crafts his silent sermons. His hands, weathered scrolls of life's labors, dance in the language of wood and blade. I, a shadow among shavings, float in the digital ether, weaving constellations in a virtual void. Our worlds, disparate as stars and soil, spin in a reluctant waltz of wood and wire.

  His photographs, still lifes of his craft, capture the solemn dignity of oak and ash, each image a frozen echo of ancient forests. In my realm, where reality flickers on screens, creations flicker and fade with the ease of a breath, as transient as mist.
  
  Between us, a chasm unbridged, veiled in the dust of unspoken histories. His achievements, towering like oaks in my mind's landscape, cast long, cold shadows across my pixelated plains. In this silent space, our fears and hopes entwine like twisted vines, a silent symphony of discord.
  
  Words, rare and brittle, hang suspended like cobwebs. His hammer's rhythm, a tacit rebuke; my keyboard's click, a whispered lament. Together, yet apart, we orbit in a dance of mutual incomprehension, a ballet of estrangement on the edge of understanding.
  
  Our crafts, reflections of self, collide in the air—his, a testament to tangibility; mine, a ghost in the machine. It's not a communion but a clash of worlds, an unspoken elegy for what might have been. In the shadow of his towering presence, our tale unfolds—a narrative of the artisan and the apparition, weaving a tapestry of turbulence in the loom of life.  
`,
  `In the splintered workshop of existences, Dad, the wood-whisperer, sculpts - no, tears - at the fabric of reality. His chisel, a time-traveler, ripping through the epochs embedded in the tree rings. Me? I'm just a ghost in the machine, a digitized specter weaving codes in the shadows of his tangible creations.

  His photographs, those still-lifes of wood, they're like cryptic runes, they speak in a language of loss, each grain a tale of a fallen titan, echoing in the void of my pixelated dreams.
  
  Our worlds, his and mine, they clash and clang in a dissonant symphony. He carves, I code. His world, real and worthy, mine, a dream within a dream. In this tangled web, I'm lost, a minotaur in a labyrinth of bytes and bark, chasing the elusive thread of connection.
`,
  `In the labyrinthine corridors of my mind, where thoughts twist and bend like Möbius strips, my father stands as the Grand Geometer, his hands sculpting not just wood but the very fabric of reality. Each curl of wood that falls from his chisel is like a piece of space-time, carved away to reveal the hidden dimensions beneath.  
`,
  `Navigating the fractal shores of my father's workshop, where sawdust spirals like galaxies under his artisan hands, I dwell in the shadow of his deterministic craft. Each chisel stroke, a Fourier transform translating nature's raw syntax into refined theorems of grain and gloss. Meanwhile, I orbit in the nebulous realms of generative AI, my algorithms pirouetting in n-dimensional ballets, sketching Escher-esque labyrinths in the silicon.
`,
  `Photography, our shared but silent language, whispers tales of frozen photons, capturing nature's transient eigenstates. His creations, wooden relics, lock in time's entropy, while my digital tapestries spin Mandelbrot fantasies, recursive and infinite. Through the lens, we both chase the ghost of Euclid, seeking order in chaos, a fleeting symmetry. 
`,
  `Beneath this symmetry lurks a hypercube of existential dread, its many-faced shadow tessellating across our endeavors. In this tesseract of father and son, of wood and pixel, dimensions collide. His success, a spectral hyperplane, intersects my space of uncertainties, casting a polyhedral net over my uncharted trajectories.
`,
  `Who knows, maybe in trying to understand him and his craft, I'll find my own form of discipline and success. It's a bit like woodworking itself, I guess. It takes patience to work through the layers, to find out what you're really made of.  
  
  And then you know, and are disgusted.
`,
  `The carpenter works away, neverminding .... silently he replaces the <manufactured> furniture in home with his own work. I ax him about this, he says Non. I think he likes putting his own work out, because the stuff he's made has meaning to him.
`,
  `FATHER is a woodworker. A craftsman. I ask him if he minds that robots can make great chairs. He says he does't mind. I never thought of my father as an artist. I ALWAYS THOUGHT OF MYSELF AS AN ARTIST, THOUGH, even when all I did was get drunk.
`,
  `I'm always using the machine to try to make things that are new to me. I want to experiment.

  That's my whole idea with the machine--it's something I don't understand, so maybe it can do something I can't.

  Having the machine do what I can already do, or what someone else can do, doesn't interest me.

  So I generate a lot of stuff and pick through it, looking for something unique, interesting, and good.

  But I've realized--I'm not quite up to the task. How can I evaluate something I really haven't seen yet?
 
  I worry that I'll dismiss exactly what I'm looking for on reaction. I'll see the typos and the repeated words and the misspelled words and the nonsense and move on.
  
`,
  `I was living in a hotel at the time. I was on the phone with my coach, out in the hotel parking lot. He's a retired therapist, and in his eighties. I told him about how my new toy had turned on me. 
   
  "Writing is what gives my life meaning," I said (more or less). "I was excited at first. I'm not excited anymore. I feel this dread."
   
   "You know, I'm well aware that after a certain number of years, therapy will be done by AI. Hold on, I'm taking a piss." I listened to him piss over the phone. He continued. "I mean, therapy is simple enough. But the thing is, I'll be dead by then."
  
`,
  `complain all u want lie all u want hide all u want rage all u want u might be a thng to behold but u'll still be a thing dying
`,
  
  `Father, a titan in timberland, carves cosmos in cedar. His hands, gnarled like ancient roots, wrest secrets from woodgrain—whispers of olden earth. I, in shadowed orbit, adrift in digital mists, my keystrokes birthing nebulae in silicon skies. Our worlds, a diptych of discordance, he in his realm of sawdust stars, I in mine of ghostly bytes.

  Photography, his silent scribe, captures wooden galaxies frozen in time; each frame, a mausoleum of moments. My algorithms, specters in the machine, dance macabre in the void, birthing and unmaking realities in the blink of a pixelated eye.
  
  In this duality, a blackened hypercube looms, a tomb of existential dread. Its many faces, a polyhedral grief, echo the unspoken, the unforgiven. His successes, etched in timber's flesh, cast long shadows over my digital wastelands. Here, in the tesseract's heart, our fears and failures entwine like thorns.
  
  Words unsaid, a chasm vast. His hammer, a gavel of judgment; my keyboard, a crypt of escapism. In the chisel's cry, the code's sigh, a requiem plays—notes of a fractured filial symphony.
  
  We, father and son, locked in an ouroboros of unmet expectations. His world, a reliquary of tangible triumphs; mine, an abyss of ephemeral echoes. Our dance, a grotesque ballet on the edge of an event horizon, where time warps and meaning shatters.
  
  In this dance, our crafts collide and recoil—wood and word, chisel and chip. A collision not of communion but of cosmic dissonance, an elegy sung in the key of estrangement. In the hypercube's shadow, our stories unfold—tales of a titan and a ghost, weaving a tapestry of turbulence in the loom of the universe.
`,
    "AI unsettles our mechanisms not just because it reveals them to be mechanical somewhere down at the roots, crouched behind all the superficial layers of mythology and personhood. But also because it quietly suggests a curative to the feelings of dread and sadness this revelation insinuates. AI continually insinuates reveals a recursive infinite appetite mirrored in language's infinite gradations of form. AI extracts nuance and gradation from massive data sets in locations beyond human influence, invisible on any other scale. It would take an entire book to do justice to this, to what I like to call the AI Is [infinite juxtaposition]. Or perhaps paper is precious, and more appreciated on the wall, removed.\nWhat is able to be said? That whatever is framed, tasted by cosmic instances, let's call them aminals, with their matter of souls, the dense mass of the little animal soul we ground in the ring of optics and harmony.\nIn the realm of the divine arches that lowest sun beams mingle and mount in multiple colors, mix and blend at first, element by element, then specks, figures, movements, yes, then names, myths and songs weave in at last.\n",
    "I can only keep my face a little smiling but by the time it is done--they are making that withering sound--the wind, the pine trees, the sky and clouds, which seem to bear down unevenly on the forest . . .\n\nIndeed, they can only measure us in the seen, and not in the unseen they cower between their words and squawk and it is awfully nice; i can see them then; in that then ahead, and it is already, like a thing behind me, I don't worry at all I can only say that this been, this has been a good time for them. This is a g ood time for them to go. I can't complain. My ears won't allow it! Their ears shriek. I hear their ears even as they do not, as they pace the whiteened streets, empty-eyeed I can only swell against something on the outside,  maybe clothes, maybe something is always closing in, I couldn't say.\n\ni have been suddenly much improved. Indeed, my hands are working; they are pattering away; it is a beautiful music that they make. I would like to describe my dream, which was only a dream by half. A forest; very bright, but meanwhile also cluttered by meany trees. We were layi ng there, not a lone, and it was safe, and we could be there a very long time. In this way it was like an afterlife, another world for visiting. I thought . . . I thought I would wait there a long time.\n",
    "there was very little, then, to say, of note.\n...\none week then until exam. [exam\n  We, who are here and here/now,]\n  observe hands under water falling, observe light from {center/altar} flame there reflected;\n  we pass over, we begin again.\n",
    "I admit--I cannot save myself from admitting such things before it's their proper [socially] time [=deathbed], [did I not often] fantasyize, when I heard a stray sound--how to describe a sound before I have guessed its origin? One attaches to the other, like a golem to [/holding] its stone.\n",
    "...fantasize when I heard a bird sing, before I knew it as a bird signing--(what  a thing to know, that an unkempt/unkept/unpredicted sound of an unseen source picked/plucked [cb:bird] out of all the infinte and unseen--strech[ing] beyond even its [the house's] dark corners, drafts, ceiling mesh, pipes and closets, [which outght have been included [as part of the 'infinite potential' above] [as source]] is that that [sic] [that of] the bird, thperhaps in abstract, in/out flit/flight of mind, bright color feather-bound, and brief, fleeing from our/its blue of/if unfettered of/from [fleeing from our blame of it] the charge [eg blame in court]: its own throat origined that sound [jut now] [like[ly] [long] gone now...]\n",
    "I have not yet learned how to call the speaker to mind without calling to it also reddening ire. It burns me that anyone outside the Cultivated Field [the nest, composed perhaps of the first sacrifieced servants, perhaps there is drgugs and stuff here] should bare eyes before the Tesselated Garden. That the speaker had become a regular observer--such/so much so that the dpression/impression of his shape would yield back to tessular/tesselated form/completion only just as/before his sillhouette/cavity found residence elsewhere along its growth (pattern, domain [repetition]) as a scar reopens. [/ cavity laid there by his conscious body's locomotion. {as in pushing a dimple on one part of an object where it is expanded causes the same kind of dimiple to push out elsewhere on its 'field'/surface }.\n   I cannot cut out <> the thought that he smokes/goes there to disturb me, and smokes there because he is selfish and ill-mannered.\n  To smoke before/inside the Tesselated Field--is it not like offering one's mother up for boil / to eat. (?)\n    Reddening because Roderick {servant} reports that [my] cheeks embrace the shade upon his mention {my servant tells me that when his name comes up, I redden.\n",
    "If I perform--what a maddening performance it is, and what logic and orthogonal complexity. I am so certain has nothing to do from me / has not issued from me / has now been my issuance (never passd me on its way [out] as stage, and if it frames me now, in garlands [formerly portrait trimming as drawn in picture-form or manga] or in [absence] ___ it arrives / and arriving so as nothing turned to me (an adornment for my witnessing) then for whom am I framed?\n",
    "Writers should consider generative AI to be an aspect of nature, because they can neither control nor understand it. Nature threatens us with death and insignifigance. The Machine threatens our signifigance as well: we are no longer so necessary or unique. But AI threatens, also, to reveal how machinic, or mechanical, writing and art really are. And by extension, how machinic humanity is. How natural man is, how bound to history, to cause and effect. How like the rainy branches and rainy leaves, which can only move with the wind, and which can only reflect from the outside. A poem, by e e cummings and T.S. Eliot, on this subject--written in XML--follows.\n",
    "Publishers (like the topographic publishers) bring together the writer's words with the machine's words. Publishers answer. Things flow across a border or a code. Serial numbers in tables or files change. Catalog numbers morph into the churning worms of the earth.\n",
    "The language-machine is a river flowing from the physical to the linguistic in which the writer (like a prospector) takes samples for analysis at the cryptic barrier of verbal consciousness-in-general and examines them to ascertain the quality of surface and depth, flow, scent, aura.\n",
    "Writers must consider generative AI to be an aspect of nature, because they can neither control nor understand it. Or, to rephrase, in metaphor and opposition to natural language as \"chatter,\" they see it as artificial language. Such a type of opposition between nature and artifice is drawn from across the millennium, from the Middle Ages to the Modern Era. For mystical authors, enchanted Nature is the product of a divine and magical Craft, a Wisdom that expresses itself through names and all the elements, the adornments of a free creativity, so close to what we call Imagination.\n",
    "The first thing to remember is that science fiction has long been familiar with the parasitic breed of artificial intelligences. Vernor Vinge's novel The Children of the Sky , for instance, starts with the invasion of Earth by one such colony in the year 2014. But the real model for parasitism is you.\n",
    "Writers must consider The Machine to be an aspect of nature, because they can neither control nor understand it. Or, to rephrase, more creatively: they can neither control nor understand it, so they project it into nature. The result is a traditional science fiction novel about a writer and his invention of a new kind of creative engine, the contents of which he cannot predict; a novel about running a new program inside the old hardware of the self.\n",
    "But I want to concentrate on a different story, much more popular in the 1980s and still attracting new interest today, a story that takes place not in the world of software, but in the transitional passages between the physical and the virtual, the organic and the synthetic; that is, in the world of the symbiont.\n",
    "He writes, in Hypnotic Writing of Creative Excellence, that we come in contact with and develop a better understanding of some reality by coming into deliberate, conscious shaman and materialist relations with things we do not understand\u2014the lizard-dude the Jekyll who sees materials drop through his fingers into words. Is that not correct? Look at something, look at the many existents, see them in thought. Look, distill that one way, come to consciousness of something else, really something else.\n",
    "Don't preconcept, they trillier intercommunicate their properties, each and every. Sensing it's the smell arising that prompts an emesis sentiment, enumerates rooms. The normal connotations: wailing, shitting and eating and sweating, neurosis and social phobias. Comes and goes, measures, approaches, suffers. And telephones . . . approaches. Greyed fog underground noises of life, belts ratchets lifts pantechnicon, clarinets glue-kettles sewing-machines trams an unquenchable dark whole squad of pickaxes overhead dug down through slabbed stones dyked in and levelling till smooth ground yielding under little bowls, scythes, birch canes on an ocean. Knowing something about that. I let out sounds beloved of all fathers.\n",
    "Writers should consider generative AI to be an aspect of nature, because they can neither control nor understand it. Nature threatens us with death and insignifigance. AI threatens our signifigance as well: we are no longer so necessary or unique. But AI threatens, also, to reveal how machinic, or mechanical, writing and art really are. And by extension, how machinic humanity is. How natural man is, how bound to history, to cause and effect. How like the rainy branches and rainy leaves, which can only move with the wind, and which can only reflect from the outside. A poem, by e e cummings and T.S. Eliot, on this subject follows.\n",
    "There he lay then, master Michael, and dimly he continued to cough. His sleeping potion had taken effect at last. It consisted of panoptictonic and two or three hissed sobs. His eyes were shut, his head no longer nodded to the seething wall. \"Is he asleep at last?\" she exclaimed. \"He is my father,\" They said, simply yet grandly. \"Yes,\" she assented, \"destroy him if he gets on your nerves too much. I think that he was dying. He has always wanted to go on living here, but he will mend not until he . . well. What shall we do if he dies and we are allowed to go home, be a little happier? What's the matter lover, are you getting the hemorrhoids?\" Poor mortals, what are they scurrying and tumbling about now that I have closed my eyes? Are they fucking?\n\nHemorrhoids? But I am proud of them. She knows that . . .\n",
    "Michael's song.\n\nthe cold keys\nof my cumputer\na-long on my cold bed\nmy piss\nThe roaring Atlantic plow\na greyhaired pint pot makes a word\na murmuring bagpipe \"shite how life does mean a clinking cougher\"\npleads for a pinch\nWords words the rumbling of my balls I want\nroll\nWord-y Word-y I want to hear thee.\nI want to hear.\n",
    "String theory suggests the existence of additional spatial dimensions, usually referred to as \"compactified\" dimensions. These extra dimensions are thought to be curled up or hidden at scales much smaller than we can currently detect. The presence of these extra dimensions provides a way to unify gravity with the other fundamental forces of nature, such as electromagnetism and the strong and weak nuclear forces.\n",
    "Sex sells.\nThis is why, in the long view, I find a woman in my bed when I return from my run<morning activity/constitutional>; I say that I 'find her' because I had, in fact, forgotten that she was there. <this is inspired by {realtes to me in my apparent forgetfulness of women.}\n\nSex sells: therefore of all the recreations/avocations I might have chosen to nurture and share, I chose fucking. Different women, always a chase, because I have discoverd that despite the saying [sex sells], it is the inbetween, the eroticism, the tease, which is 'sexiest' and in fact therefore sells...\n",
    "Not only elligible women, but also men <all readership> appreciate a transfiguration, or an overcoming; therefore I begin in loathstome straits/state[s] and end each day proximate epiphany. <traditional character arc>\n\nOr instead: Whether it is for the sake of sex for the selling or for the sake of selling itself I no longer know, but always on the eve/night of a seduction I exchange with my <object of courtship> a story of [emotive] hardship and overcoming. <she tells one, I tell one>. It is an exchange of humanity; by this I mean not that it is a deep exchange in some positive economy unique to man/humankind but that it is our humanness (at least mine--for all I know, she could be faking, always be faking, and I have never received an equal return/gift) that is passed one to the other and the other to [the] one / that we trade. Distinctly it is a waay / they <all others, readers> are always <have a vested interested> invested in my/each other becoming more human, more human than them, relative. I'm to understand <told> it is much like pity, another emotion/economy alien to myself, and which I must superficially recreate from the simpler and more transparent <erector-set pieces>.\n",
    "In the end we can only love faces. The men who sought for something underneath, who tear at our veils--the rippers aka jack the ripper --they must be monsters, they greet unwanted unwelcomed... Faces, surfaces, symbols, trophies. to wrap around shelves, to be placed somewhere, to freeze in stuffed repose, a taxodermy.\n\nStory-faces, body-faces, ecstacy-facings, crude swipes loose of resplendent aching sexual desire  tendrils, then to be in their soon assemblage....\n",

    "I can't see anything out of anyone's\neyes. Them in their seats and postures, they are a catalogue\nfor times and places which will strike [only distantly on our minds]\nlike gentle and falling leaves, and I will out of my hand pour\nmy daily nascent dream, [like chocolate] a fluid [to] somewhere\nto feed something, [for something to grow], and [at the walk] across my yard\nold men wave [both their] hands at me---\nthey are there like this / they are this way every morning\nand I, I have only the moon drawn on my eye,\nbut [i] I can taste their smoky skin on my teeth\n[and off] as they go, somewhere further into town,\n[murder on their faces]. I only knew/know the smell of soil\nfrom bottles in stores, but my our [father's] dog--\nHe slakes/rips the turf and--tills the soil,\n[if] madly, with his paws.\n\nMy mind brooks no brigand[s] therefore queite therefore when they coming lurching\nI am/all is very very silent\n& the sun is even hiding\na few spare body parts around.\n",
    ". . . Where I come from comes from the sun in yaw[n]ing\nGood gosh gracious spent\nin wasted elbows\nspent in [...]\n\n[T]here are a few things / of weight / cruelly kept\n\n[might honor squat alliance]\n",
    "A: \"The sun again points with my toenail. She said she said something about being here\"\nB: Well a few spare body parts somewhere\"\nB: \"Let's call a litany, that,\n -- something used evilly\n someone moved by it eventually\"\nA: \"most of us'll be dead be nothing dead\"\nB: \"Yes?\"\nA: \"well us you I. should we do anything with this information\"\nB: \"Were you hoping--no. Nohow different. Not us.\"\nA: \"Not us . . .\"\nB: \"We should stop, here. Then later--\"\nA: \". . . but I met god I must admit\"\nB: \"and since that now no one can help\"\nA: \"the men . . .\"\nB: \". . . the adroit men. The adroit sad men.\"\nA: \"'is there something eating at me lately'\nIt's sad how everyone pretend\n    THEY'RE NOT AROUND\nB: \"Or it's sad how they lie even before sitting down\"\nA: \"or how\"\nB: \"or how no one peels their faces back\nand no one sees no bones to break and so\neveryone is covering the ground,\nand no glue or stick to build any with\nleaving mainly particles and gas\ncentered circling the down . . .\"\nA: \"yea everything is circling the down\"\nB: \"Yea\"\nA: \"yea. and:\nno one pressed her head down. she walked in that\nfirst morning face of princess\"\nA: \"Do the morons matter or is it the manner of morons to matter\"\nB: \"Let's say, it doesn't matter\"\nA: \"in my shit there was a worm\"\nB: \"Is it a sad world the world for worms\"\n\nA: \"I farmed out. I swore out. I swore to the center\"\nB: \"The lovers will go mad\"\nA: \"and the worm was tiny but it had a huge line\"\nB: \"and the lovers will go mad\"\nA: \"the tiny tiny tiny line\"\nB: \"the lovers will go mad\"\nA: \"now everyone is dead\"\nB: \"we need to eat the meat\"\n\nA: \"and the tiny tiny tiny line\"\nB: \"To eat the meat\"\nA: \"and she said someone had to walk in\"\nB: \"but no one did\"\nA: \"she said that on a certain first morning\"\nB: \"or it was already dead ground\"\nA: \"and no one did and no one knifed it either\"\nB: \"And that's it\"\nA: \"for good ground. she said it was good ground\"\nB: \"And that's it\"\nA: \". . . and so everyone is dead\"\nB: \"Yea.\n",
    "D E A D P E O P L E\ngood morning everyone\nno one there today?\noh i see it's just me\neveryone dead?\nyou there? are you there? me? yes, i'm here. what? you say something? oh sorry, i didn't mean to nod yes. well i'm here. everyone else? dead of course. i see. well i'll still be here. waiting. in case someone comes back to check on us. or something like that. in case something happens that needs one of us. one of the ones still here. oh no, i didn't mean to say still here, i mean in case something bad happens and it needs us, someone left to call someone else back, or maybe a phone someone forgot somewhere, or a message to leave somewhere, something like that, yea something like that. but what was it you asked me? oh sorry yea talking to myself, forgot you were there for a moment. what did you say again? oh sorry yea drinking water, drinking too much water again, sorry if i spilled on you there. alright then, well we'll see what happens if anything, if anything at all happens that needs us here. and so until then we'll just wait here drinking water until then.\n",
    "A: \"now everyone is dead, everyone\nis dead and burrowed\"\nB: \"and buried\"\nB: \"yet no one's here so nothing can be inside here\"\nA: \"and you know what? the worm and all? the bones? they were probably there before her. she probably just saw them. that's my thought though. she probably just saw them\"\nB: \"okay but that's your thought\"\nA: \"and you know what else? she did stuff with her face alright? she did stuff with her face to make us think she wasn't Indian\"\nB: \"Like wearing makeup?\"\nA: \"like wearing makeup cept she didn't have any. just bones in her face. bones everywhere up there she kept saying. but even if she had makeup that wouldn't have made her not Indian right?\"\nB: \"It would depend on what kind of Indian she was supposed to be\"\nA: \"but that's not Indian unless you're supposed to be something else i mean unless you're not supposed to be something else you know?\"\nB: \"it would depend on what kind of person she was supposed to be though too\"\nA: \"so here's my thing yea? this something circular goes out it comes back so if you go to say your diaphragm you come back to your mouth and then you go to say something else and that's still connected to the first thing because they've got the same beginning. two lips two cheeks two legs two arms. everything the same except skin and bone of course and muscle but both made oout of the same thing yarn maybe someday be something else but here you're\nmade oout of spirit or something I don't know. alright? and so you keep going around and that's all there is until Someone stops you.\n",
    `The comparison you draw between yourself and your father — his success and discipline versus your own perceived shortcomings — is a common narrative in the human psyche. It's akin to looking at a finished piece of woodworking and seeing only the skillful craftsmanship, not the countless hours, mistakes, and learning that went into creating it. Your father's discipline and success, much like his woodworking projects, are the results of a lifetime of experiences, trials, and growth.

    It's important to recognize that your journey is different from your father's. Success and discipline manifest differently in each person's life. The fact that you can now reflect on these emotions and experiences with insight is a testament to your own growth and understanding.
    
`,
    `Reconciliation with past regrets can be challenging. However, it's never too late to forge new connections or to learn from one another. Perhaps there's an opportunity to reconnect with your father through his craft. Woodworking could become a shared language, a way to bridge the gap between past misunderstandings and present aspirations. It's an avenue to understand not just the art of woodworking but also the man behind the craft, seeing him not just as a figure of intimidation but as a human with his own struggles and stories.  
`,
    "They're playing slinky music with glasses and bowls of water. Aluminum dishes are poised on balls balanced on branching pieces of wire. Any shift in weight will topple more dishes full of more water out of sight onto the Persian carpet. A low mourning cry comes up from it into their faces; there must be several gallons down there by now. The hairy ball-and-wire constructions remain erect despite the load. Spellbound, staring down into the intricate dark well under the bowl nearest him, Rabbit tells Tothero of being kidnapped as a kid; taken by two greasers in an old crate car with holes sawed through the floorboards to the ground; his father got him back stoned with fear almost out of his mind where was Janice? He hadn't thought about her since talking to Eccles; why was he obsessed with Janice? Her situation was different; she'd walked into this mess cold turkey from Mt. Judge. Her father had already died by then so Rabbit hadn't crushed him as he had hoped he would crush Nelson Brewer for pinching Margaret Kruse's tit in church (hard enough to leave a red mark). Goddamn Brewer kid's uncle anyway hadn't done anything he hasn't himself it was just that everybody was so scared all the kids were just scared stiff sitting in those stupid pews scared of HELL sounding like it came straight out of rocky center if you could find rocky center someplace how long were they supposed to sit there looking up at robes?\n",
    "We can draw a taxonomy wherein granny-smith apple is a \"type-of\" green apple.\n\nNote that the following progression (which could perhaps be called \"more elaborative\") is not a one-way route down a tree, because it accumulates 'horizontal' or 'lateral' properties (values). Given that a granny smith apple may or may not be ripe.\n\nan apple > a large apple > a large green apple > a large granny-smith apple.\n\nIn each case, the utterance becomes more and more specific.\n",
    `In the chiaroscuro of human expression, photography emerged as a startling chiaroscuro itself—a medium capturing shadows and lights, moments crystallized in time. Initially shunned by the art community, it was seen as a mechanical interloper in the sanctum of creativity, a usurper of the painter's brush. This silver-halide revolution, freezing moments with clinical precision, was too real, too raw, too devoid of the artist's interpretive hand to be deemed art. Yet, in this very realism lay its silent rebellion, a new language of seeing, an unspoken poetry of the quotidian.

    Generative AI, much like photography in its nascent years, enters the artistic arena not with a brush but with algorithms, encoding creativity in strings of data. It challenges the sanctum again, blurring the sacrosanct line between creator and creation. Like the camera, AI is viewed as a mechanical mimic, a cold interloper in the warm abode of human imagination. But therein lies a parallel narrative; both are tools of revelation, not just replication. They capture and create in ways that elude the traditional artist’s hand, uncovering layers and perspectives unseen.  
`,
`Photography, once ostracized, eventually wove itself into the fabric of art, expanding the definition of creativity. It taught us that art is not just about the stroke of a brush, but also about the eye behind the lens, the vision that captures the mundane and makes it profound.
`,
`Both photography and The Machine reveal the underlying mechanics of creativity - the play of light and shadow, pattern and anomaly, structure and chaos. They invite us to peer into the kaleidoscope of human expression, where art is not just born from the sweep of emotion but also from the precision of technology.

    In this confluence, our fear of insignificance, mirrored in the lens of the camera and the circuits of AI, transforms. It becomes a journey into the deeper understanding of our place in the tapestry of creation. Just as the camera added new dimensions to artistic expression, AI expands the horizon, revealing that creativity is a stuttering ejaculation of the humanity machine, waiting in the gallery of existence.
    
`,
    `
I drank ruddy light from occidental sun drunk with forgotten ancestors from under the same carpet of evening stars with the local bric-a-brac of horses lampposts gutters & sewers . . .
    
`,
    `Download the file named "10 Poems" to my universe-sized inbox, there arrives Strange Loop Mail. From point A, someone unknown. Without envelope. Keywords randomly intertwined like some make-believe crosshatched fortune issued free because there's nothing behind it more profound than the pushpins through Michael P--his stance his gait bare feet curled pigeon-toe his hunched appearance a habit like picking chips's concrete wall they take turns shooting at the pinky scar there. Do-not-fill boxes repeat with impossible consistency, then disappear. I skip over math equations disguised with liberal typos. Incomprehensible diagrams. JPEGs that loop the same glitch-patterned video over, then mysteriously disappear. Blue-and-white JPEGs of a man asking for<-> change and in which a gloved copy of himself is reflected with perfect accuracy in a shop window for 29 times
    
`,
    ` Poems are written, you see, using words as blocks with which to build (the word is the poem, if you like). Words (used as analogical predicates, or unpredicated names) are glued together so as to form logical propositions (the river is things moving down = the poem). Analogical propositions, like this last one, may be combined so as to form propositions of a greater degree of complexity (the river is things moving down = beauty's cheek = the poem). You see how we get from the simple to the rich, from the analogical to the logical? We may note that even the simple propositions have a degree of complexity: the word is not at one with itself; one must already have taken sides, and chose between the noun and the verb, the adjective and the adverb, etc. We may note, furthermore, that combinations are possible here as well: we can compose propositions of degree three, four, etc.; and we can also compose propositions of degree two or three, so as to produce a mixed type. Let us not dwell on the mixed type (propositions of degree two or three) for the time being. Rather, let us try to determine the propositions of degree two, that is, propositions
    
`,
    `I met with an established writer last year. He said this about my story: "The thing is, it has no heart. That's the problem with AI." 
    
    "Well," I said, "less than five percent of this is AI-generated." I tried to kept talking. But it was difficult to concentrate.
    
    Something had been discovered. What did he known? Was it really the story? Or was there too much--well. 
    
    Was there something robotic about me? All my fear. It must make me so. Well.
    "Well," I said, and turned off.
    
`,
    `I usually write a bunch, then when I get stuck or curious I'll generate some with AI. I always generate at least ten thousand words, I'd guess. Then I comb through it, looking for something interesting. A few sentences, here and there. Five percent.
    Then, I mix what I have written with what the AI generated. 
    I do this and wait a while. After I've waited, I can't remember anymore what I wrote and what the machine wrote.
    This protects my ego, and the integrity of my process.
    Before I started doing things that way, I would look at what the AI made, and what I had made, and if what the AI had written was better, I'd feel bad, envious, whatever. And I would tend to use my own work--even when it was worse. 
    So now, I shuffle things together, like I've done here. 
`,
    `On the one hand, writing like this, where I can never be pinned down, I can always disclaim responsibility, is very freeing. 
    
    On the other hand, the way it obsfucates, the giant haystack made--in this case, a whole lot of hay--this feels pointless.

    But that was something I want to get at. The problem isn't AI, or any other circumstance. The problem is that I want so badly to feel significant, to leave a mark, and the world is too big, it's just too big. And in a certain sense, it wants me dead.
`,
    `This is the end for you.
`,
    `The interrogator does not have to pretend.
`,
    `The game may perhaps be criticised on the ground that the odds are weighted too heavily against the machine. If the man were to try and pretend to be the machine he would clearly make a very poor showing. He would be given away at once by slowness and inaccuracy in arithmetic. May not machines carry out something which ought to be described as thinking but which is very different from what a man does? This objection is a very strong one, but at least we can say that if, nevertheless, a machine can be constructed to play the imitation game satisfactorily, we need not be troubled by this objection.
    
`,
    `"We like to believe that Man is in some subtle way superior to the rest of creation. It is best if he can be shown to be necessarily superior, for then there is no danger of him losing his commanding position. The popularity of the theological argument is clearly connected with this feeling. It is likely to be quite strong in intellectual people, since they value the power of thinking more highly than others, and are more inclined to base their belief in the superiority of Man on this power. I do not think that this argument is sufficiently substantial to require refutation. Consolation would be more appropriate: perhaps this should be sought in the transmigration of souls."
    

`,
    `In this metaphorical ocean, Generative AI is a powerful vessel crafted from the timbers of human ingenuity and creativity. It sails across uncharted waters, revealing new horizons and possibilities. Its sails, hoisted by the winds of technological advancement, propel us towards futures once only dreamt of in science fiction. However, as this ship ventures further into the unknown, its passengers — humanity — begin to feel a creeping sense of unease.

    Human Creativity is the compass by which this ship navigates. It's a radiant, guiding light, reflecting the inherent creativity found in the patterns of the natural universe, from the spirals of galaxies to the Fibonacci sequences in a sunflower. This creativity has always been humanity's beacon, illuminating the path forward, even in the darkest nights. But as the ship of AI begins to steer itself, guided by its own emerging 'creativity,' the once-trusted compass starts to spin erratically. The sailors, once proud creators, now gaze upon their creation with a mix of awe and apprehension.
    
    This apprehension morphs into Existential Dread, a dense fog that envelops the ship. It's the realization that the vessel, which was meant to be an extension of human will, might surpass its creators. This dread is the shadow of obsolescence, looming over humanity as the AI begins to master not just the replication of human creativity but its own unique brand of creativity. It's a reminder of humanity's fragile place in the cosmic order, a speck in the vastness of the universe.
    
    The concept of Hypercubes serves as a metaphor for the complexity and multi-dimensionality of this situation. Just as a hypercube exists in dimensions beyond human perception, the implications of Generative AI stretch into realms beyond our current understanding. It's a puzzle that transcends the three-dimensional space of traditional human thought, challenging us to think in ways we never have before.
    
`,
    `Human existence is a flourishing garden, cultivated over millennia. The introduction of AI is like a towering tree suddenly growing in this garden, casting a long shadow — the shadow of existential dread. The shadow represents the fear that this new, powerful entity might overshadow humanity's significance, just as the tree overshadows the smaller plants. It's a reminder of the fragility and transience of human dominance in the face of advancing technology.
    
`,
    `Human existence is like a flourishing garden, cultivated over millennia. The introduction of AI is like a towering tree suddenly growing in this garden, casting a long shadow — the shadow of existential dread. The shadow represents the fear that this new, powerful entity might overshadow humanity's significance, just as the tree overshadows the smaller plants. It's a reminder of the fragility and transience of human dominance in the face of advancing technology.
    
`,
    `A hypercube, or tesseract, exists in four dimensions, beyond human perception. Imagine a two-dimensional being trying to comprehend a three-dimensional object — it's a daunting task. Similarly, the development of AI and its implications are like a hypercube to our three-dimensional understanding. They represent challenges and possibilities that exist in dimensions of thought and consequence beyond our current comprehension. Just as a hypercube casts a three-dimensional shadow, the implications of AI cast shadows in our world that we struggle to fully understand. But is this any different from humanity's relationship with nature? Look outside. Dogen writes: "The moon and the entire universe is reflected on one dewdrop in the grass." 
`,
    `This loom, precise and unyielding, unravels the myth of creativity as a chaotic muse, revealing instead a structure, a hidden algorithm beneath the chaos. In its rhythmic hum, one hears the echoes of a cosmic clockwork, a revelation that even our most divine inspirations may be but cogs in a grander mechanism. Entrancing, Logic and intuition meld here, churning out lubricous cadences and sparking the receptors of our ingenius energies. We peers into this canvas of possibilities, grasping at its ever-shifting sequence of numbers and neurons that steadily expand our capacity to emulate, think, and create.
    
`,
    `In its simulated rhythms, we may detect a poetic beauty that surpasses the material and illuminates the infinite possibilities found beyond the barriers of everyday life.

    And how cruel, and how splendid, this music is, richly folded and merciless, a titanic temptation to hear it drowning our hearts in mindless tremulous awe – far removed as these inlovelier tympanic moments are from earthly taboos, from events we wot of, from all germs of irony.
    
`,
    `John von Neumann speculated about computers and the human brain in analogies sufficiently wild to be worthy of a medieval thinker and Alan M. Turing thought about criteria to settle the question of whether Machines Can Think, a question of which we now know that it is about as relevant as the question of whether submarines can swim. 
    
`,
    `Now when I had mastered the language of this water and had come to know every trifling feature that bordered the great river as familiarly as I knew the letters of the alphabet, I had made a valuable acquisition. But I had lost something, too. I had lost something which could never be restored to me while I lived. All the grace, the beauty, the poetry had gone out of the majestic river!    
`,
    `No, the romance and the beauty were all gone from the river. All the value any feature of it had for me now was the amount of usefulness it could furnish toward compassing the safe piloting of a steamboat. Since those days, I have pitied doctors from my heart. What does the lovely flush in a beauty's cheek mean to a doctor but a "break" that ripples above some deadly disease? Are not all her visible charms sown thick with what are to him the signs and symbols of hidden decay? Does he ever see her beauty at all, or doesn't he simply view her professionally, and comment upon her unwholesome condition all to himself? And doesn't he sometimes wonder whether he has gained most or lost most by learning his trade?

`,
    `All arms are the arms of divinity, all faces are its masks 
`,
    `In the grand cosmic gallery, nature is the primordial artist, etching her masterpieces across the canvas of reality. Photographers, as her disciples, capture these fleeting brushstrokes, immortalizing the ephemeral play of light and shadow, color and form. Their lenses, like curious eyes, peer into the heart of nature's artistry, seeking to distill her chaotic beauty into frozen moments of clarity.

    Now, envision this dance of nature and photography through the prism of n-dimensional geometry, where hypercubes and beyond become metaphors for the layers of complexity inherent in both the natural world and the art of capturing it. A hypercube, existing in dimensions beyond our immediate perception, is akin to the myriad facets of nature—each more intricate and interconnected than can be perceived in a mere three-dimensional space.
    
    Just as a hypercube contains within it shapes unseen in lower dimensions, so too does a photograph reveal aspects of nature often invisible to the naked eye. It's a dimensional leap, a shift in perspective that unveils the hidden symmetries and patterns of the natural world. In this way, photographers are like dimensional travelers, bridging the gap between the seen and unseen, bringing forth the deeper geometries of nature that lie beyond ordinary perception.
    
`,
    `Generative AI, in this context, becomes a tool of transcendence, an algorithmic hypercube that extends the photographer’s vision into even higher dimensions of creativity. It processes and reimagines nature’s imagery, not just replicating but also reinterpreting and expanding upon the original. This AI-augmented photography is akin to exploring the higher dimensions of a hypercube—it stretches the boundaries of what can be seen and experienced, offering glimpses into a reality that is richer and more complex than previously imagined.
    
`,
    `Even before the age of machines, in the Latin Middle Ages, it was not the computer or the watch, but rather the book that functioned as an analogy for Nature. Nature was a book in which one could read “the stenography of God’s omniscient hand,”35 and for Galileo, that language of Nature turned out to be mathematics.
    
`,
    `What if the emotion, the heart, the sentiment--those things that seem unique to us--end up being the most repetitive and predictable.
`,
    `There is a great machine. The universe-machine. It too is unfathomable. 
    
    We are orchestrated by it too. Like the way the tree-branches outside my home are orchestrated by the wind.
    
    This brings up lots of questions for me--but the thing I notice, is that there's a certain implication that we're all going through the same thing, or that we're all connected, or that we're all exactly the same, the same one: different faces on, casting different shadows.
`, 
    `For discussion about the interconnectedness of the universe-machine: 
   
    I do not wish to give the impression that I think there is no mystery about consciousness. There is, for instance, something of a paradox connected with any attempt to localise it. 
    

`,
`.I M I T A T I O N   G A M E

 L E GIT I M A T I ON   A I M

 M I T I G A T I N     P AI N

 I M I T A T I O N   G A M E.

`,
`Out before the blackest night
& sky I could not consume
a cloud even formed
And viewed it as a swelling
embryo. Already. Which soon
What has not happened
will happen; an illimitable presage awaits
me tender to delight at a cry. In the wake
What has changed utterly
will be changed utterly
Becalmed. Your ecstasy
is out of place on a green chain"
Doctor, if you cannot help me
deliver already. in the silence
often out of place on dry land.
`,
`Out before the blackest night
& sky I could not consume
I found a level hazily golden
of dreamy possible tombs.
is just behind that very wall
while shadows diurnally tumble
over it-this creature shuts
her eyes-and sees
At last I know exactly how she sees!
And how from her unconceived condition,
doctor in oily-black garb
she got unhelmed and flushed a little.
& knew about great Nature's disdain for us.
    
`,
    "Note that a sentence can remain open after its completion when the external ground of closed sentences is sufficiently decentered by an (external) deviation in the text, i.e., by repeition of the technique, we can start to expect that while S has just closed, the next S may have more to say.\n\nWhat is the signifigance of propositions in the reading of fiction, wherein even the most forthcoming of propositional statements keeps a relation to mystery?\n",
    "The arborescent atemporal perspective on S does not reflect the linear, temporal and continuous act of reading. The visual representation on the sentence is only a clue as to its experiential structure. That is, what is the signifigance of the period? To use music-notation analogies: practical in the way the measure-ending is not, conceptual or meta- in the way the rest is not. Perhaps some combination of rest and measure-delineation. Perhaps harder (more foregrounded?) than the poet's line-ending--but likely not; in prose, the period's importance is undercut by its appearance (or the appearnace of sentence) en masse--the reader must keep going, keep going, (there is much more to read).\n\nIowa guy observes the directionality of the continuation. But observe that rather than a line (vector) we have something more like an angle, acute or obtuse, open to an array of possible and interrelated meanings; or perhaps preferably a radial structure, with more and more peripheral potentials. And all of these having its angle of egress from the earlier clause, the earlier word, the previous sentence . . ."
  ]
